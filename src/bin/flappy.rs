...
// colors
const BACKGROUND_COLOR: Color = Color::srgb(0.15, 0.15, 0.20);
const PLATFORM_COLOR: Color = Color::srgb(0.25, 0.60, 0.45);
const PLAYER_COLOR: Color = Color::srgb(0.90, 0.60, 0.20);
...
// platform generation
const PLATFORM_SPAWN_AHEAD: f32 = 800.0;
const PLATFORM_DESPAWN_BEHIND: f32 = 200.0;
const PLATFORM_MIN_WIDTH: f32 = 60.0;
const PLATFORM_MAX_WIDTH: f32 = 200.0;
const PLATFORM_MIN_GAP: f32 = 150.0;
const PLATFORM_MAX_GAP: f32 = 300.0;

const NEEDLE_COLOR: Color = Color::srgb(0.80, 0.30, 0.30); // red so player notices needle obstacles
...
impl PlatformBundle {
        ...
        Self {
            ...
            transform: Transform {
                translation: Vec3::new(
                    x,
                    WINDOW_BOTTOM_Y + (scale.y / 2.0), // base sits on floor
                    0.0,
                ),
                scale,
                ..Default::default()
    ...
    fn new_at_y(x: f32, scale: Vec3, y_bottom: f32) -> Self {
        Self {
            sprite: Sprite {
                color: NEEDLE_COLOR, // different color so player recognizes needle obstacles
                ..Default::default()
            },
            transform: Transform {
                translation: Vec3::new(
...
fn generate_platforms(
    ...
    while level_state.furthest_x < spawn_target {
        ...
        commands.spawn(PlatformBundle::new_at_y(
            x,
            Vec3::new(width, gap_center_y, 1.0),
            0.0,
        ));
